...
export default function AIActivityFinder({ destinations, selected, onToggle, onDestinationToggle, language }: Props) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => { gsap.from('.activity-reveal', { opacity: 0, y: 35, stagger: .1, duration: .8, ease: 'power3.out', scrollTrigger: { trigger: root.current, start: 'top 78%' } }); gsap.fromTo(background.current, { yPercent: -8, scale: 1.08 }, { yPercent: 16, scale: 1.18, ease: 'none', scrollTrigger: { trigger: root.current, start: 'top bottom', end: 'bottom top', scrub: 1.2 } }); }, root);
    return () => ctx.revert();
  }, []);
  ...
  const pointerMove = (event: React.PointerEvent<HTMLElement>) => { if (matchMedia('(prefers-reduced-motion: reduce)').matches) return; const x = event.clientX / innerWidth - .5, y = event.clientY / innerHeight - .5; gsap.to(background.current, { x: x * 32, y: y * 20, duration: 1.5 }); gsap.to(glow.current, { x: x * -70, y: y * -45, duration: 1.8 }); };
  ...
  return <section ref={root} onPointerMove={pointerMove} className="relative isolate mb-10 overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-slate-950 shadow-[0_35px_100px_rgba(0,0,0,.35)]">
    <img ref={background} src={photo?.url || fallbackImage} onLoad={() => gsap.fromTo(background.current, { opacity: .05 }, { opacity: .42, duration: 1.3 })} onError={event => { event.currentTarget.onerror = null; event.currentTarget.src = fallbackImage; }} referrerPolicy="no-referrer" alt={`${types[0] || 'Sri Lanka'} activity`} className="absolute inset-[-4%] h-[108%] w-[108%] object-cover will-change-transform" />
    <div className="absolute inset-0 bg-[linear-gradient(105deg,rgba(2,8,18,.96),rgba(2,8,18,.79)_52%,rgba(2,8,18,.68))]" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-transparent to-slate-950/20" /><div ref={glow} className="absolute right-[12%] top-[18%] h-64 w-64 rounded-full bg-cyan-300/15 blur-[100px]" />
